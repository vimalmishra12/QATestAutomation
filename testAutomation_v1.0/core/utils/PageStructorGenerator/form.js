...
function generateTestcase(pageSelectorFile, inputFile, pageSelectorGroup, appShellPageCheck) {
    ...
    file2.write("var " + inputFile + "= require('../../pages/ExperienceApp/" + inputFile + ".page.js');");
    if (appShellPageCheck)
    file2.write("\nvar appShell = require('../../pages/ExperienceApp/appShell.page');");
    file2.write("\nvar sts;\n\nmodule.exports = {\n");
    for (var i = 0; i < pageSelectorFile.length; i++) {
        if ((pageSelectorFile[i].tagName).toLowerCase().includes("button")) {
            console.log("ENG_" + (inputFile.substring(0, 4)).toUpperCase() + "_TC_" + testCaseNumber + " :   async function (testdata) { \n");
...
function generatePageHeader(PageTemplate, param1, param2, appShellPageCheck) {
    ...
    if (param1)
        console.log(PageTemplate.header[param1]);
...
function generatePageSelector(pageSelectorFile, inputFile) {
    for (var i = 0; i < pageSelectorFile.length; i++) {
        console.log(pageSelectorFile[i].Label + ": selectorFile.css.ComproC1." + inputFile + "." + pageSelectorFile[i].Label + ",\n");
        file.write(pageSelectorFile[i].Label + ": selectorFile.css.ComproC1." + inputFile + "." + pageSelectorFile[i].Label + ",\n");
    }
}
