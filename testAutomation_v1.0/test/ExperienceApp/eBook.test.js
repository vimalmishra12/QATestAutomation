...
await assertion.assertEqual(sts.pageStatus, true, "eBook page status mismatch");
},
...
await assertion.assertEqual(sts, true,"cqaEbookEvolveDropdown are not Clicked");
},

TST_EBOO_TC_7 :   async function (testdata) { 
sts = await eBook.click_cqaTestEbookOnlyAssets();
await assertion.assertEqual(sts, true,"cqaTestEbookOnlyAssets are not Clicked");
},
...
await assertion.assertEqual(sts.cqa_ebook_evolve, testdata.cqa_ebook_evolve,"cqa_ebook_evolve Values is not as expected.");
},

TST_EBOO_TC_11 :   async function (testdata) { 
sts = await eBook.getData_eTextToolBar(testdata);
await assertion.assertEqual(sts.contentButton, testdata.contentButton,"contentButton Values is not as expected.");
await assertion.assertEqual(sts.toolsButton, testdata.toolsButton,"toolsButton Values is not as expected.");
await assertion.assertEqual(sts.homeButton, testdata.homeButton,"homeButton Values is not as expected.");
await assertion.assertEqual(sts.pageNumber, testdata.pageNumber,"pageNumber Values is not as expected.");
},
...
await assertion.assertEqual(sts.myNotes, testdata.myNotes,"myNotes Values is not as expected.");
},

TST_EBOO_TC_14 :   async function (testdata) { 
sts = await eBook.getData_ebookContent(testdata);
await assertion.assertEqual(sts.cqaEbookEvolveDropdown, testdata.cqaEbookEvolveDropdown,"cqaEbookEvolveDropdown Values is not as expected.");
},

TST_EBOO_TC_15 :   async function (testdata) { 
sts = await eBook.getData_ebookContentDropdown(testdata);
await assertion.assertEqual(sts.cqaTestEbookOnlyAssets, testdata.cqaTestEbookOnlyAssets,"cqaTestEbookOnlyAssets Values is not as expected.");
},
